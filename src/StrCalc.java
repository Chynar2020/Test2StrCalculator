...
public class StrCalc {
    ...
    public static void main(String[] args) {
        System.out.println("Введите выражение в скобках где a и b - строки а x - число  от 1 до 10 включительно  и Enter ");
//      Считываем строку userInput которую ввёл пользователь
        String userInput = scanner.nextLine();
//      Создаём пустой символьный массив длиной 26 символов
        char[] schar = new char[26];
//
        ArrayList<String> strBlock00 = new ArrayList<>(10);
        for (int i = 0; i < userInput.length(); i++) {
            schar[i] = userInput.charAt(i);
            if (schar[i] == '+') {
                operation = '+';
            }
            if (schar[i] == '-') {
        ...
        if (blocks.length == 5) {
            ...
            if (result.length() > 40) {
                System.out.println(result.substring(1, 41) + "...");
            }
    ...
    public static String calculated(String num1, String num2, char op) {
        ...
        switch (op) {
            case '+':
                result = num1 + num2;
                if (result.length() > 41) {
                    result= result.replace(result.substring(41), "...");
                }
                break;
